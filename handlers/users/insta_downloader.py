...
from .misk import get_response, prepare_urls
...
async def insta_download(msg: types.Message):
    ...
    if vid_url:
        video = ("/ ".join(vid_url)).split()
        # print(video)

    if pic_url:
        picture = ("/ ".join(pic_url)).split()
        # print(picture)
    ...
    if video:
        for item in video:
            album.attach_video(video=item)
    if picture:
        for item in picture:
            album.attach_photo(photo=item)
